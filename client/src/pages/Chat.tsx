...
export function Chat() {
  ...
  /**
   * Set the width according the compression of the data normalized vs the denormalized 
   *
  ...
  */
  ...
  const messagesUpdateListener = () => {
    axios
      .get<IMongoPopulatedMessages[]>(
        `http://localhost:8080/api/messages/list/${token}`
      )
      .then((response) => {
  ...
  const selectedMessages = sentToSelected
    .concat(receivedFromSelected)
    .sort((a, b) => {
            ...
            {Object.keys(receivedMessages).map((user_id, idx) => {
              ...
              return (
                <li
                  className="chat"
                  id={user_id}
                  onClick={(e) => setSelectedChat(e.currentTarget.id)}
                >
                  <img className="chat-image" src={user.avatar} alt="" />
                  <div className="chat-info">
                    ...
                    <span className="chat-text">
                      {receivedMessages[user_id][lastMessage].message}
                    </span>
                  </div>
                </li>
              );
          ...
          <div className="selected-chat">
            <header className="chat-header">
              <img
                ...
                alt=""
              />
              <span>{`${selectedUserChat.name} ${selectedUserChat.surname}`}</span>
            </header>
            ...
            <div className="chat-footer">
              ...
              <button className="send-message-btn" onClick={sendMessage}>
                <img
                  style={{ width: "100%" }}
                  src="https://static.thenounproject.com/png/3061866-200.png"
                  alt=""
                />
              </button>
            </div>
