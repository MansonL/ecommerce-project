...
export const Config = {
  ...
  JWT_SECRET:
    process.env.JWT_SECRET || "secret string for jwt signature & verify",
  JWT_EXPIRATION_TIME: Number(process.env.JWT_EXPIRATION_TIME) || 60 * 15, // 15 minutes
  SESSION_SECRET:
    process.env.SESSION_SECRET || "secret string for express-sessions",
  SESSION_COOKIE_TIMEOUT:
    process.env.SESSION_COOKIE_TIMEOUT || "timeour for session cookies",
  ...
  GOOGLE_CLIENT_ID:
    process.env.GOOGLE_CLIENT_ID || "admin google api oauth2 client_id",
  GOOGLE_CLIENT_SECRET:
    process.env.GOOGLE_CLIENT_SECRET || "admin google api oauth2 client_secret",
  GOOGLE_REFRESH_TOKEN:
    process.env.GOOGLE_REFRESH_TOKEN ||
    "adming google account oauth2 refresh_token",
  FACEBOOK_APP_ID: process.env.FACEBOOK_APP_ID || "admin facebook app_id",
  FACEBOOK_APP_SECRET:
    process.env.FACEBOOK_APP_SECRET || "admin facebook app_secret",
  CLOUDINARY_CLOUD_NAME:
    process.env.CLOUDINARY_CLOUD_NAME || "admin cloudinary cloud name",
  CLOUDINARY_API_KEY:
    process.env.CLOUDINARY_API_KEY || "admin cloudinary api_key",
