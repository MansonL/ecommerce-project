...
export class MongoCart implements DBCartClass {
  ...
  async add(
    ...
    try {
      ...
      if (cartDoc) {
        const product = cartDoc.products.find(
          (product) => product.product.toString() === product_id
        );
        if (product) {
          const canAddExistingOne = await Utils.validateCartModification(
            product_id,
          ...
          if (canAddExistingOne) {
            ...
            return ApiError.badRequest(
              `Not enough stock of the desired product.`
            );
        } else {
          const canAddNewOne = await Utils.validateCartModification(
            product_id,
            quantity
          );
          if (canAddNewOne) {
        ...
        const canAdd = await Utils.validateCartModification(
          product_id,
          quantity
        );
        if (canAdd) {
  ...
  async delete(
    ...
    try {
      const cartDoc = await this.cart.findOne({ user: user_id });
      if (cartDoc) {
        const deleted = cartDoc.products.filter(
          (product) => product.product.toString() === product_id
        );
        if (deleted.length > 0 && deleted[0].quantity >= quantity) {
          const newProducts =
            deleted[0].quantity === quantity
          ...
          // The error was caused by the incorrect quantity to delete
          return ApiError.badRequest(
            `The desired amount of the product to delete is greater than the amount stored in the cart`
          );
        else return ApiError.notFound(ECartErrors.ProductNotInCart);
      } else {
        return ApiError.notFound(ECartErrors.EmptyCart);
      }
    } catch (error) {
