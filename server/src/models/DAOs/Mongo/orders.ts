      ...
      if (type != null && _id != null) {
        if (type === "user") {
          ...
          if (doc) {
            ...
            if (docPopulated instanceof ApiError) return docPopulated;
            else return docPopulated[0].orders;
          } else return ApiError.notFound(EOrdersErrors.NoOrdersCreated);
        } else {
          ...
          } else return ApiError.notFound(EOrdersErrors.OrderNotFound);
        }
      } else {
        ...
        if (docs.length > 0) {
          ...
          else return populatedAddressDocs;
        } else return ApiError.notFound(EOrdersErrors.NoOrdersCreated);
      }
    } catch (error) {
      ...
      if (doc) {
        ...
        if (docPopulated instanceof ApiError) return docPopulated;
          ...
          return {
            ...
            data: docPopulated[0].orders.find(
              (orderPopulated) => orderPopulated._id == order._id
            ) as IOrderPopulated,
          };
      } else {
      ...
      if (doc) {
        ...
        if (docPopulated instanceof ApiError) return docPopulated;
          ...
          return {
            ...
            data: docPopulated[0].orders.find(
              (populatedOrder) => String(populatedOrder._id) == order_id
            ) as IOrderPopulated,
          };
      } else return ApiError.notFound(EOrdersErrors.OrderNotFound);
    } catch (error) {
