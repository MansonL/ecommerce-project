...
export class Utils {
    ...
    static getUserCartOrDefault = async (user_id: string): Promise<IMongoCart> => {
        ...
        if(userHasCart instanceof ApiError){
            ...
            if(userCart instanceof ApiError)
                ...
                return userCart.data as IMongoCart
        }else{
            ...
            if(DBProducts.length === ids.length){
                ...
                const valid = DBProducts.every(DBproduct => {
                    if(order[DBproduct._id] > DBproduct.stock) 
                        orderProducts.forEach(orderProduct => {
                            if(String(orderProduct.product_id) == DBproduct._id) invalidProductAmount.push(orderProduct.product_title)
                    })
