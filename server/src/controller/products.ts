...
class ProductController {
  ...
  async getByCategory(
    ...
    } else next(ApiError.badRequest(`Category name needed.`));
  }
  ...
  async save(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (error) next(ApiError.badRequest(error.message));
      ...
      const result: ApiError | CUDResponse = await productsApi.addProduct(
        product
      );
      if (result instanceof ApiError) next(result);
      else {
    ...
    if (isValidObjectId(product_id)) {
      if (error) next(ApiError.badRequest(error.message));
        ...
        if (firstResult instanceof ApiError) next(firstResult);
          ...
          const result: CUDResponse | ApiError =
            await productsApi.updateProduct(product_id, newProperties);
          if (result instanceof ApiError) next(result);
  ...
  async delete(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (isValidObjectId(product_id)) {
      ...
      if (firstResult instanceof ApiError) next(firstResult);
        ...
        const result: CUDResponse | ApiError = await productsApi.deleteProduct(
          product_id
        );
        if (result instanceof ApiError) next(result);
        else res.status(201).send(result);
  ...
  async query(req: Request, res: Response, next: NextFunction): Promise<void> {
    ...
    if (firstResult instanceof ApiError) next(firstResult);
       ...
       */
      const maxDBPrice = await Utils.getMaxStockPrice(
        firstResult as IMongoProduct[],
        "price"
      );
      const maxDBStock = await Utils.getMaxStockPrice(
        firstResult as IMongoProduct[],
        "stock"
      );
