...
class OrdersController {
  async getByAdmin(
    ...
    if (user_id) {
      ...
      const result: IMongoOrderPopulated[] | IOrderPopulated[] | ApiError =
        await ordersApi.get(undefined, undefined);
  ...
  async createOrder(
    ...
    if (isValidOrder instanceof ApiError) next(isValidOrder);
      ...
      if (result instanceof ApiError) next(result);
        ...
        const adminsResult = await usersApi.getAdmins();
        if (adminsResult instanceof ApiError) res.send(adminsResult);
        else {
          const adminsSubject = `[NEW ORDER]: ${user.name} ${user.surname} has made an order.`;
          const adminsTo = adminsResult.join(", ");
          const customerMail = EmailUtilities.createHTMLOrderEmail(
            (result.data as IOrderPopulated).products,
            EmailUtilities.addressHTMLFormat(selectedAddress),
            total,
            ["You have made an order!", "Your order will be delivered to"]
          );
          const adminsMail = EmailUtilities.createHTMLOrderEmail(
            (result.data as IOrderPopulated).products,
            EmailUtilities.addressHTMLFormat(selectedAddress),
            total,
            ["A new order was made!", "Address"]
          );
    ...
    if (order_id && newStatus) {
      if (isValidObjectId(order_id)) {
        if (
          ...
          const result: CUDResponse | ApiError = await ordersApi.modifyOrder(
            order_id,
            newStatus
          );
          if (result instanceof ApiError) next(result);
          ...
          next(
            ApiError.badRequest(
              `Valid status to assign: paid, delivering or completed `
            )
          );
      } else next(ApiError.badRequest(EProductsErrors.IdIncorrect));
    } else {
      next(
        ApiError.badRequest(
          `Please type an order id and a new status for the order.`
        )
      );
    }
  }
