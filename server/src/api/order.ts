...
export class OrdersApi {
    ...
    async get(type: 'user' | 'order' | undefined, _id: string | undefined): Promise<IMongoOrderPopulated[] | IOrderPopulated[] | ApiError> {
        const result : IMongoOrderPopulated[] | IOrderPopulated[] | ApiError = await this.orders.get(type, _id);
        return result
    }

    async create(order: IOrder, user_id: string): Promise<CUDResponse | ApiError> {
        const result: CUDResponse | ApiError = await this.orders.create(order, user_id);
        return result
    }

    async modifyOrder(order_id: string, newStatus: 'created' | 'paid' | 'delivering' | 'completed'): Promise<CUDResponse | ApiError> {
        const result : CUDResponse | ApiError = await this.orders.modifyStatus(order_id, newStatus);
        return result
    }
};
